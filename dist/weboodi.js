"use strict";
const getLocalStorage = (key, initialValue = "[]") => JSON.parse(localStorage.getItem(key) || initialValue);
const getListFromLocalStorage = (key, initialValue = "[]") => getLocalStorage(key, initialValue).filter(notEmpty);
...
const getDuplikaattiKurssit = () => getListFromLocalStorage("duplikaattiKurssit");
...
const map = (list, keys) => list.reduce((acc, item) => [
    ...acc,
...
const findFromKurssiTietokantaRecurse = ({ db, lyhenne }) => Object.keys(db).reduce((acc, key) => {
    const courseFound = !acc.length &&
        isArray(db[key]) &&
        db[key].find(({ keys }) => keys.map(toLowerCase).includes(lyhenne.toLowerCase()));
    return (acc ||
        (isArray(db[key])
            ? courseFound
                ? courseFound.name
...
const findFromKurssiTietokanta = (lyhenne) => findFromKurssiTietokantaRecurse({ db: kurssitietokanta, lyhenne });
    ...
    tooltips: {
        callbacks: {
            label: (tooltipItem, data) => {
                ...
                const labelValue = tooltipItem.datasetIndex || isFloat(value / 10) ? value : value / 10;
...
const draw = ({ id, labels, datasets, type = "bar", customTooltip = false, customTicks = false, }) => {
    ...
    new Chart(elem, {
        ...
        options: Object.assign({}, (customTooltip && teeHienoTooltip()), { scales: {
                yAxes: [
                    Object.assign({}, (customTicks && {
                        gridLines: {
                            drawBorder: false,
                            color: chartColors,
                        },
                    }), { ticks: Object.assign({ beginAtZero: true }, (customTicks && {
                            max: maxValue,
                            stepSize,
                        })) }),
                ],
...
const doCss = () => `
  ...
  `;
const yolohtml = ({ duplikaattiKurssit, perusOpinnot, aineOpinnot, pääaine, sivuaineet, }) => `
...
const pitäisköDomRakentaa = () => !!document.querySelector(suoritusTableSelector);
const createDom = ({ duplikaattiKurssit, aineOpinnot, perusOpinnot, pääaine, sivuaineet, }) => {
...
const createCoursesArray = (target) => target.value
    .split(",")
    .map(putsaaTeksti)
    .filter(notEmpty);
...
const setDailyCumulativeNoppas = ({ pvm, op }) => (jee) => {
    const today = jee.pvm === pvm
        ? {
            cumulativeOp: op + jee.cumulativeOp,
            op: op + jee.op,
        }
        : null;
    return Object.assign({}, jee, today);
};
const groupThemCourses = (stuff) => stuff
    .reduce((initial, item) => findPvm(initial, item.pvm)
    ? initial.map(setDailyCumulativeNoppas(item))
    : [...initial, item], [])
    .map((item) => (Object.assign({}, item, { op: item.op <= 10 ? item.op * 10 : item.op, realOp: item.op })));
const putsaaTeksti = (str) => str.replace(/&nbsp;/g, " ").trim();
const muutaArrayKivaksiObjektiksi = ([lyhenne, kurssi, op, arvosana, pvm = "01.01.1970", luennoitsija,]) => ({
    pvm,
    kurssi,
    lyhenne,
    luennoitsija,
    op: Number(poistaSulut(op)),
...
const lasketaanpaLopuksiKumulatiivisetNopat = (initial, item, i) => [
    ...initial,
    Object.assign({}, item, { cumulativeOp: item.op + (i && initial[i - 1].cumulativeOp) }),
];
const hommaaMeilleListaAsijoistaDommista = () => [
    ...document.querySelectorAll("[name=suoritus] + table + table:not(.eisei) table.eisei tbody tr"),
];
const makeSomeStuff = (duplikaattiKurssit) => hommaaMeilleListaAsijoistaDommista()
...
const takeUntil = (list, n) => list.reduce((initial, item, i) => (i < n ? [...initial, item] : initial), []);
const sum = (a, b) => a + b;
const average = (list) => list.reduce(sum, 0) / list.length;
const annaMulleKeskiarvotKursseista = (stuff) => stuff
    .filter((item) => !isNaN(item.arvosana))
    .map((item, i, list) => (Object.assign({}, item, { keskiarvo: average(takeUntil(map(list, "arvosana"), i + 1)).toFixed(2) })));
const annaMulleKeskiarvotTietyistäKursseista = ({ kurssit, stuff }) => annaMulleKeskiarvotKursseista(stuff.filter(({ lyhenne }) => kurssit.includes(lyhenne)));
const rakennaListaLuennoitsijoista = (initial, item) => [
    ...
    ...item.luennoitsija
        ...
        .map(putsaaTeksti)
        .filter(notEmpty)
        .map((luennoitsija) => (Object.assign({}, item, { luennoitsija }))),
];
const haluaisinTietääLuennoitsijoista = (stuff) => stuff
    .reduce(rakennaListaLuennoitsijoista, [])
    .map((item, i, arr) => {
    const luennot = arr.filter(({ luennoitsija }) => luennoitsija === item.luennoitsija);
    const arvosanat = map(luennot.filter((item) => !isNaN(item.arvosana)), "arvosana");
    const keskiarvo = average(arvosanat);
    return Object.assign({}, item, { kurssimaara: luennot.length, luennot: {
            arvosanat,
    ...
    .reduce((initial, item) => initial.find(({ luennoitsija }) => luennoitsija === item.luennoitsija)
    ? initial
    : [...initial, item], []);
const poistaAvoinKurssiNimestä = (kurssi) => kurssi
    .replace("Avoin yo:", "")
    .replace("Open uni:", "")
    .trim();
...
const haluanRakentaaSanapilvenJa2008SoittiJaHalusiSanapilvenTakaisin = (stuff) => map(stuff, "kurssi")
...
const removeDuplicateCourses = (coursesDone) => (acc, item) => acc.find(courseNamesMatch(item.kurssi)) ||
    ...
    ? acc
    : [...acc, item];
const removeAvoinFromKurssiNimi = (item) => (Object.assign({}, item, { kurssi: item.kurssi.replace("Avoin yo: ", "").replace("Open uni: ", "") }));
...
const drawOpintoDonitsi = ({ id, stuff, data }) => {
    const [coursesDone, coursesNotDone] = partition(data, (lyhenne) => !stuff.find((course) => lyhenne === course.lyhenne));
    const opintoData = [
    ...
    const greatSuccess = coursesDone.length === opintoData.length ? "All done, nice!" : "";
    setHtmlContent({
        id: `${id}-progress`,
    ...
    drawPie({
        ...
        backgroundColor: opintoData.map(({ done }) => done ? "lightgreen" : "lightgray"),
    });
};
const findOpintoByLyhenne = ({ opinnot, lyhenne }) => opinnot.find((item) => lyhenne === item.lyhenne);
const hommaaMulleKeskiarvotTietyistäOpinnoistaThxbai = ({ stuff, keskiarvot, kurssit, }) => {
    if (!kurssit.length) {
        return [];
    }
    ...
    return keskiarvot.reduce((initial, item) => {
        ...
        if (jeejee) {
            return [...initial, Object.assign({}, jeejee, { fromOpinnot: true })];
        }
        const mitäs = initial.filter(({ fromOpinnot }) => fromOpinnot).reverse()[0];
        return [...initial, Object.assign({}, mitäs, { arvosana: 0 }) || item];
    }, []);
};
const rakenteleDataSetitKeskiarvoChartille = ({ keskiarvot, keskiarvotPerusopinnoista, keskiarvotAineopinnoista, }) => [
    notEmpty(keskiarvot) && Object.assign({ label: "Kurssien keskiarvo", data: map(keskiarvot, "keskiarvo") }, style),
    notEmpty(keskiarvotPerusopinnoista) && Object.assign({ label: "Perusopintojen keskiarvo", data: map(keskiarvotPerusopinnoista, "keskiarvo") }, styleBlue),
    notEmpty(keskiarvotAineopinnoista) && Object.assign({ label: "Aineopintojen keskiarvo", data: map(keskiarvotAineopinnoista, "keskiarvo") }, styleGreen),
].filter(isTruthy);
const rakenteleDataSetitNoppaChartille = (grouped) => [
    ...
    Object.assign({ label: "Suoritukset", data: map(grouped, "cumulativeOp") }, style, { type: "line" }),
].filter(isTruthy);
const drawGraphs = ({ stuff, keskiarvot, keskiarvotPerusopinnoista, keskiarvotAineopinnoista, }) => {
...
const rakenteleDateObjekti = ([paiva, kuukausi, vuosi]) => new Date(vuosi, kuukausi - 1, paiva);
...
const laskeLukukausienNopat = (prev, { pvmDate, op }) => {
    ...
    return Object.assign({}, prev, { [vuosiJuttu]: op + (prev[vuosiJuttu] || 0) });
};
...
const laskeKumulatiivisetKuukausienNopat = (prev, { pvmDate, cumulativeOp }) => (Object.assign({}, prev, { [luoKivaAvainReducelle(pvmDate)]: cumulativeOp }));
const ryhmitteleStuffKivasti = ({ fn, stuff }) => stuff.sort(sorttaaStuffLukukausienMukaan).reduce(fn, {});
const hemmettiSentäänTeeDataSetti = ({ label, data, secondDataSet }) => [
    Object.assign({ label,
        data }, styleBlue),
    secondDataSet && Object.assign({}, secondDataSet, { type: "line" }, styleGreen),
].filter(isTruthy);
const piirräPerusGraafiNopille = ({ id, label, labels, data, secondDataSet }) => draw({
    id,
...
const piirteleKuukausittaisetJututJookosKookosHaliPusJaAdios = ({ kuukausiGroups, kumulatiivisetKuukaudetGroups, }) => {
    piirräPerusGraafiNopille({
...
const sorttaaLuennoitsijatKeskiarvonMukaan = (a, b) => b.luennot.keskiarvo - a.luennot.keskiarvo || b.kurssimaara - a.kurssimaara;
...
const laitaHyvaksytytSuorituksetDomiinJeps = ({ kurssimaara, hyvMaara, hyvOp, }) => {
...
const luoTilastoaAineidenKeskiarvoista = ({ key, data }) => `${key} ${data.laitos} keskiarvo on ${isNaN(data.keskiarvo) ? "hyv" : data.keskiarvo} ja painotettu keskiarvo on ${isNaN(data.painotettuKeskiarvo) ? "hyv" : data.painotettuKeskiarvo}`;
const piirraRandomStatistiikkaa = ({ kurssimaara, luennoitsijamaara, op, openUniMaara, openUniOp, hyvMaara, hyvOp, maxKuukausi, keskiarvo, painotettuKeskiarvo, pääaine, sivuaineet, }) => {
    setHtmlContent({
        id: "opintojen-maara",
    ...
    setHtmlContent({
        ...
        content: `Olet käynyt ${luennoitsijamaara} eri luennoitsijan kursseilla, ${(kurssimaara / luennoitsijamaara).toFixed(2)} kurssia per luennoitsija, ${(op / luennoitsijamaara).toFixed(2)} op per luennoitsija.`,
    });
    setHtmlContent({
    ...
    if (sivuaineet) {
        setHtmlContent({
            id: "sivuaineet-data",
            content: sivuaineet
                .map((data) => luoTilastoaAineidenKeskiarvoista({ key: "Sivuaineesi", data }))
                .join("<br>"),
        });
    }
...
const piirraRumaTagipilvi = (words) => {
    ...
    const content = Object.keys(words)
        ...
        .map(({ fontSize, key, count }) => `<span style="font-size: ${fontSize}px;" title="${key} on mainittu ${count} kertaa suorituksissasi">${key}</span>`)
        .join(" ");
    setHtmlContent({
        id: "tagipilvi",
        content,
...
const nameIncludesAvoinYo = (name) => name.includes("avoin yo") || name.includes("open uni");
const laskePainotettuKeskiarvo = (data) => {
    const arvosanallisetOpintosuoritukset = data.filter(({ arvosana }) => !isNaN(arvosana));
    return (arvosanallisetOpintosuoritukset.reduce((acc, { op, arvosana }) => acc + arvosana * op, 0) / map(arvosanallisetOpintosuoritukset, "op").reduce(sum, 0)).toFixed(2);
};
const laskeStuffistaHalututJutut = ({ stuff, key }) => stuff.reduce((acc, item) => (Object.assign({}, acc, { [item[key]]: acc[item[key]] ? acc[item[key]] + 1 : 1 })), {});
const laskeKuinkaMontaMitäkinNoppaaOnOlemassa = (stuff) => laskeStuffistaHalututJutut({ stuff, key: "op" });
const laskeKuinkaMontaMitäkinArvosanaaOnOlemassa = (stuff) => laskeStuffistaHalututJutut({ stuff, key: "arvosana" });
const piirräGraafiNoppienTaiArvosanojenMäärille = ({ id, label, data }) => draw({
    id,
    type: "bar",
    labels: Object.keys(data).map((key) => `${label} ${isNaN(key) ? "hyv" : key}`),
    datasets: [
        Object.assign({ label: "Suorituksia", data: Object.values(data) }, styleBlue),
    ],
});
const parsiLaitoksenKoodi = (lyhenne) => lyhenne
    .replace(/^(ay|a)/i, "")
    .replace(/(-|_)[\d\D]+/i, "")
    .replace(/[\d]+/, "");
const grouppaaEriLaitostenKurssit = (stuff) => stuff.reduce((acc, kurssi) => {
    ...
    const painotettuKeskiarvo = laskePainotettuKeskiarvo(edellisenKierroksenData ? edellisenKierroksenData.kurssit : [kurssi]);
    const dataJeejee = {
        courseCount: 1,
        op,
        kurssit: [kurssi],
    ...
    return Object.assign({}, acc, { [laitos]: edellisenKierroksenData
            ? Object.assign({}, edellisenKierroksenData, dataJeejee, { courseCount: edellisenKierroksenData.courseCount + 1, op: edellisenKierroksenData.op + op, kurssit: [...edellisenKierroksenData.kurssit, kurssi] }) : dataJeejee });
}, {});
const piirräLaitosGraafit = (data) => {
    ...
    draw({
        ...
        datasets: [
            Object.assign({ label: "Kursseja", data: map(dataset, "courseCount") }, style),
            Object.assign({ label: "Nopat", data: map(dataset, "op") }, styleBlue),
            Object.assign({ label: "Keskiarvo", data: map(dataset, "keskiarvo") }, styleGreen),
            Object.assign({ label: "Painotettu keskiarvo", data: map(dataset, "painotettuKeskiarvo") }, styleGreen),
        ],
    });
};
const start = () => {
    ...
    const { duplikaattiKurssit, perusOpinnot, aineOpinnot, pääaine, sivuaineet, } = hommaaMatskutLocalStoragesta();
    createDom({
        duplikaattiKurssit,
        perusOpinnot,
        aineOpinnot,
    ...
    const { keskiarvotPerusopinnoista, keskiarvotAineopinnoista, } = laskeKeskiarvot({ stuff, keskiarvot, perusOpinnot, aineOpinnot });
    const luennoitsijat = haluaisinTietääLuennoitsijoista(stuff);
    const suositutSanat = haluanRakentaaSanapilvenJa2008SoittiJaHalusiSanapilvenTakaisin(stuff);
    ...
    const maxKuukausi = Object.entries(kuukausiGroups).find(([_, op]) => op === maxKuukausiNopat);
    const { keskiarvo } = [...keskiarvot].pop();
    const painotettuKeskiarvo = laskePainotettuKeskiarvo(stuff);
    const arvosanatGroupattuna = laskeKuinkaMontaMitäkinArvosanaaOnOlemassa(stuff);
    ...
    const sivuaineidenMenestys = Object.values(laitostenKurssit).filter(({ laitos }) => contains(mapInvoke(sivuaineet, "toUpperCase"), laitos.toUpperCase()));
    ...
    piirraRandomStatistiikkaa({
        ...
        hyvOp: map(stuff.filter(({ arvosana }) => isNaN(arvosana)), "op").reduce(sum),
        maxKuukausi,
        keskiarvo,
        painotettuKeskiarvo,
